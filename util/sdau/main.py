...
def get_class_info(username, password):
    ...
    while flag < 0:
        ...
        if times > 10:
            ...
            return
    text = HTML_HEAD + \
        s.get("http://jw.sdau.edu.cn/xkAction.do?actionType=6").text
    ...
    for items in BeautifulSoup(str(soup.find_all('table')[7]), 'lxml').find_all('tr', {'class': 'odd'}):
        ...
        if len(item) < 10:
            week_num = re.findall(r'\d*-\d*',item[0].get_text())[0].split('-')
            week_num = list(range(int(week_num[0]), int(
                week_num[len(week_num)-1])+1))
            ...
            week_num = item[11].get_text().strip().replace(
                '周', '').replace('上', '').split('-')
            week_num = list(range(int(week_num[0]), int(
                week_num[len(week_num)-1])+1))
